...
def update_soul_txt(public_url):
    with open("jatin4.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in jatin4.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("jatin4.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated jatin4.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update jatin4.txt on VPS: {str(e)}")
